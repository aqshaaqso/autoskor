    ...
    h(
      'p',
      {
        className: `truncate text-xs ${
          jobName ? 'font-medium text-primary-700' : 'text-slate-500'
        }`,
      },
      jobName ? `Pekerjaan: ${jobName}` : 'Tidak ada pekerjaan',
    ),
  )
}
  ...
  return h(
    ...
    h(
      ...
      h(
        ...
        `${workers.filter((worker) => worker.currentDocument).length} dari ${engine.workerCount ?? workers.length} worker aktif`,
      ),
    ),
    workers.length > 0
      ? h(
